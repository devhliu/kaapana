...
app = Dash(
    __name__, external_stylesheets=EXTERNAL_STYLES, title="RateMe", url_base_pathname=URL_BASE_PATH
)
        ...
        dbc.Row(
            [
                ...
                dbc.Col(
                    [
                        dbc.Progress(
                            id="progress_bar", value=0, label="0%", color="success", striped=False
                        ),
                        dbc.Row(
                            [
                                dcc.Graph(
                ...
                dbc.Col(
                    [
                        dbc.InputGroupText(
                            "Check missing annotations", className="bg-info text-white p-2"
                        ),
                        dcc.Dropdown(
...
@app.callback(
    Output("dummy_update", "children"), Input("upload", "contents"), prevent_initial_call=True
)
...
@app.callback(
    Output("download", "data"), Input("download_button", "n_clicks"), prevent_initial_call=True
)
def download_data(_):
...
def update_files(_, data_path: str):
    ...
    if DATA.data_reader.n_images > 20:
        marks = {
            i + 1: str(i + 1)
            for i in range(0, DATA.data_reader.n_images, int(DATA.data_reader.n_images / 10))
        }
    else:
