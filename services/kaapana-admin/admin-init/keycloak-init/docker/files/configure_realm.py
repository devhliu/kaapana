...
class KeycloakHelper:
    def __init__(self):
        ...
        self.master_access_token = self.get_access_token(
            self.KEYCLOAK_USER, self.KEYCLOAK_PASSWORD, "https", self.KEYCLOAK_HOST, 443, False, "admin-cli"
        )
    ...
    def __configure_keycloak(self, url: str, request=requests.post, payload={}, **kwargs):
        for key, val in kwargs.items():
            payload[key] = val
        r = request(url, verify=False, json=payload, headers={"Authorization": f"Bearer {self.master_access_token}"})
