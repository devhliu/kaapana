...
def add_extension_to_dict(
    ...
    if extension_dict["version"] not in global_extensions_dict[extension_name].available_versions:
        ...
        global_extensions_dict[extension_name].latest_version = sorted(
            list(global_extensions_dict[extension_name].available_versions.keys()), key=LooseVersion, reverse=True
        )[-1]
...
def add_info_from_deployments(extension_info: schemas.KaapanaExtension, result_list: List[schemas.KaapanaExtension]):
    dep_exists = False
            ...
            if len(states_w_indexes) == 0:
                ...
                for ind, ext in states_w_indexes:
                    ...
                    if len(dep) > 1 or len(tgz) > 1:
                        logger.error(
                            "ERROR in recently_updated_states dep or tgz, dep: {0}, tgz: {1}".format(dep, tgz)
                        )
...
def collect_all_tgz_charts(keywords_filter: List, name_filter: str = "") -> Dict[str, Dict]:
    ...
    """
    logger.debug(
        "collect_all_tgz_charts with keyword filter: {0}, name filter: {1}".format(keywords_filter, name_filter)
    )
...
def get_recently_updated_extensions() -> List[schemas.KaapanaExtension]:
    ...
    to_remove = []
    logger.debug(
        "get_recently_updated_extensions called with global_recently_updated {0}".format(global_recently_updated)
    )
