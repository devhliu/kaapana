    ...
    try:
        ...
        if settings.recent_update_cache and check:
            ...
            if len(states_w_indexes) == 0:
                ...
                logger.debug(
                    "updating cache, states_w_indexes {0}".format(states_w_indexes)
                )
        ...
        if check:
            ...
            # nothing updated recently, return cached
            logger.info(
                "skipping list generation -> return global_extensions_dict_cached"
            )
            return global_extensions_dict_cached
...
def collect_all_tgz_charts(
    ...
    if name_filter != "":
        if len(collected_tgz_charts) > 0:
            logger.debug(
                "returning collected_tgz_charts {0}".format(collected_tgz_charts)
            )
...
def helm_show_chart(name=None, version=None, package=None) -> Dict:
    ...
    if package is not None:
        ...
        helm_command = (
            f"{helm_command} {settings.helm_extensions_cache}/{name}-{version}.tgz"
        )
...
def update_extension_state(state: schemas.ExtensionStateUpdate = None):
    ...
    if key not in global_extension_states:
        logger.warning(
            "{0} is not already in global_extension_states, adding a new entry".format(
                key
            )
        )
        global_extension_states[key] = schemas.ExtensionState.construct(
        ...
        logger.info(
            f"updating extension state to {0} from {1}".format(state.state, prev_state)
        )
