...
def helm_prefetch_extension_docker(helm_namespace=settings.helm_namespace):
    ...
    for name, payload in image_dict.items():
        ...
        success, stdout, helm_result_dict = pull_docker_image(release_name, blocking=True, **payload)
        if success:
...
def helm_install(
    ...
    if "extension_params" in payload:
        logger.debug("found extension_params in payload {0}".format(payload["extension_params"]))
...
def helm_delete(
    ...
    cached_extension = [x for x in helm_helper.global_extensions_dict_cached if x["releaseName"] == release_name]
    try:
        if len(cached_extension) == 1:
            ...
            if release_version is not None:
                logger.debug("fetching version {0} in available versions {1}".format(release_version, versions))
...
def execute_update_extensions():
    ...
    for idx, kube_helm_collection in enumerate(settings.kaapana_collections.split(";")[:-1]):
        ...
        payload.update({"release_name": release_name, "sets": {"kube_helm_collection": kube_helm_collection}})
