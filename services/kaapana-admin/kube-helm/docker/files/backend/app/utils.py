...
def helm_delete(
    ...
    # delete version
    helm_command = (
        f"{settings.helm_path} -n {helm_namespace} uninstall {helm_command_addons} {release_name}"
    )
        ...
        if "post-delete" in stdout:
            ...
            for release in json_out:
                if release["name"] == release_name:
                    logger.debug(
                        "found the release in helm ls results, release: {0}".format(release)
                    )
                    found = True
                    break
            if not found:
...
def execute_update_extensions():
    ...
    logger.info(
        "chart info for update extensions {0}, {1}".format(chart["name"], chart["version"])
    )
    ...
    logger.info(
        "split kaapana collections {0}".format(settings.kaapana_collections.split(";")[:-1])
    )
    for idx, kube_helm_collection in enumerate(settings.kaapana_collections.split(";")[:-1]):
        ...
        payload.update(
            {"release_name": release_name, "sets": {"kube_helm_collection": kube_helm_collection}}
        )
