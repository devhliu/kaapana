...
def install_nnunet():
    ...
    rinstall = requests.post(
        ...
        json={"name": "nnunet-workflow", "version": "03-22", "keywords": ["kaapanaworkflow"]},
    )
    assert (
        str(rinstall.status_code)[0] == "2"
    ), "/helm-install-chart did not respond with right status code, got {0} instead".format(rinstall.status_code)
    print(rinstall.text)
...
def delete_nnunet():
    ...
    rdel = requests.post(
        ...
        json={"helm_command_addons": "", "release_name": "nnunet-workflow", "release_version": "03-22"},
    )
    assert (
        str(rdel.status_code)[0] == "2"
    ), "/helm-delete-chart did not respond with right status code, got {0} instead".format(rdel.status_code)
    assert (
        rdel.text == "Successfully uninstalled nnunet-workflow"
    ), "/helm-delete-chart returned different response {0}".format(rdel.text)
