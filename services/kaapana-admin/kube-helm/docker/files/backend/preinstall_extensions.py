...
for extension in preinstall_extensions:
    ...
    try:
        ...
        logger.error(f"Skipping {extension_path}, since we had problems installing the extension {e}")
        errors_during_preinstalling = True
...
for _ in range(7200):
    ...
    for release_name in releases_installed.keys():
        ...
        releases_installed[release_name] = (
            True if all_successful(set(kube_status["status"] + [status["STATUS"]])) == "yes" else False
        )
