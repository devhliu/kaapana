...
def create_and_update_client_kaapana_instance(
    ...
    allowed_dags = json.dumps(
        get_dag_list(
            only_dag_names=False, filter_allowed_dags=client_kaapana_instance.allowed_dags
        )
    )
...
def get_jobs(
    db: Session, instance_name: str = None, status: str = None, remote: bool = True, limit=None
):
...
def update_external_job(db: Session, db_job):
    if db_job.external_job_id is not None:
        same_instance = (
            db_job.owner_kaapana_instance_name == settings.instance_name
        )  # will be the same !
        db_remote_kaapana_instance = get_kaapana_instance(
            db, instance_name=db_job.owner_kaapana_instance_name, remote=True
        )
...
def create_cohort(db: Session, cohort: schemas.CohortCreate):
    if cohort.kaapana_instance_id is None:
        ...
        db_kaapana_instance = (
            db.query(models.KaapanaInstance).filter_by(id=cohort.kaapana_instance_id).first()
        )
...
def get_cohorts(
    db: Session, instance_name: str = None, limit=None, as_list: bool = True, username: str = None
):
    print(username)
...
def get_experiment(db: Session, experiment_id: int = None, experiment_name: str = None):
    if experiment_id is not None:
        ...
        db_experiment = (
            db.query(models.Experiment).filter_by(experiment_name=experiment_name).first()
        )
    ...
    if experiment.experiment_status != "abort":
        for (
            ...
            # or update db_jobs on remote kaapana_instance
                ...
                raise HTTPException(
                    status_code=404, detail="Job updating while updating the experiment failed!"
                )
