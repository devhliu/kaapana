...
def get_kaapana_instances(db: Session, filter_kaapana_instances: schemas.FilterKaapanaInstances = None):
    if filter_kaapana_instances.instance_names:
        ...
        return (
            ...
            .filter(
                models.KaapanaInstance.remote == filter_kaapana_instances.remote,
                models.KaapanaInstance.allowed_dags.contains(filter_kaapana_instances.dag_id),
            )
            .all()
        )
...
def create_and_update_client_kaapana_instance(
    db: Session, client_kaapana_instance: schemas.ClientKaapanaInstanceCreate, action="create"
):
...
def create_and_update_remote_kaapana_instance(
    db: Session, remote_kaapana_instance: schemas.RemoteKaapanaInstanceCreate, action="create"
):
    utc_timestamp = get_utc_timestamp()
...
def create_job(db: Session, job: schemas.JobCreate):
    ...
    if (
        ...
        minio_urls = HelperMinio.add_minio_urls(job.conf_data["federated_form"], db_kaapana_instance.instance_name)
    ...
    try:
        ...
        return (
            db.query(models.Job)
            .filter_by(
                external_job_id=db_job.external_job_id, owner_kaapana_instance_name=db_job.owner_kaapana_instance_name
            )
            .first()
        )
    ...
    if db_kaapana_instance.remote is False and db_kaapana_instance.automatic_job_execution is True:
        job = schemas.JobUpdate(
            **{"job_id": db_job.id, "status": "scheduled", "description": "The worklow was triggered!"}
        )
...
def delete_job(db: Session, job_id: int, remote: bool = True):
    ...
    if (db_job.experiment.kaapana_instance.remote != remote) and db_job.status not in ["queued", "finished", "failed"]:
        raise HTTPException(
            status_code=401, detail="You are not allowed to delete this job, since its on the client site"
        )
...
def update_job(db: Session, job=schemas.JobUpdate, remote: bool = True):
    ...
    if (
        ...
        dag_id_and_dataset = check_dag_id_and_dataset(
            db_job.kaapana_instance, conf_data, db_job.dag_id, db_job.owner_kaapana_instance_name
        )
    ...
    if (db_job.kaapana_instance.remote != remote) and db_job.status not in ["queued", "finished", "failed"]:
        raise HTTPException(
            status_code=401, detail="You are not allowed to update this job, since its on the client site"
        )
...
def sync_client_remote(
    ...
    for db_incoming_job in db_incoming_jobs:
        ...
        incoming_experiment = (
            [schemas.Experiment(**experiment.__dict__).dict() for experiment in db_incoming_experiment][0]
...
def get_remote_updates(db: Session, periodically=False):
    ...
    for db_remote_kaapana_instance in db_remote_kaapana_instances:
        ...
        job_params = {"instance_name": db_client_kaapana.instance_name, "status": "queued"}
        if same_instance is True:
            incoming_data = sync_client_remote(
                db=db,
                remote_kaapana_instance=schemas.RemoteKaapanaInstanceUpdateExternal(**update_remote_instance_payload),
                **job_params,
            )
        else:
            remote_backend_url = f"{db_remote_kaapana_instance.protocol}://{db_remote_kaapana_instance.host}:{db_remote_kaapana_instance.port}/kaapana-backend/remote"
        ...
        for incoming_experiment in incoming_experiments:
            ...
            if db_incoming_experiment is None:
                ...
                incoming_experiment["involved_kaapana_instances"] = incoming_experiment["involved_kaapana_instances"][
                    1:-1
                ].split(
                    ","
...
def create_identifier(db: Session, identifier: schemas.Identifier):
    db_identifier = models.Identifier(identifier=identifier.identifier)
...
def create_cohort(db: Session, cohort: schemas.CohortCreate):
    ...
    if cohort.cohort_query and not cohort.cohort_identifiers:
        ...
        db_cohort_identifiers: List[models.Identifier] = [
            create_identifier_if_not_present(db, identifier) for identifier in cohort.cohort_identifiers
        ]
...
def update_cohort(db: Session, cohort=schemas.CohortUpdate):
    ...
    db_identifiers = [create_identifier_if_not_present(db, identifier) for identifier in cohort.cohort_identifiers]
    ...
    if cohort.action == "ADD":
        ...
        db_cohort.cohort_identifiers = [
            identifier for identifier in db_cohort.cohort_identifiers if identifier not in db_identifiers
        ]
...
def create_experiment(db: Session, experiment: schemas.ExperimentCreate):
    if experiment.kaapana_instance_id is None:  # experiment has a kaapana_instance_id?
        ...
        db_kaapana_instance = (
            db.query(models.KaapanaInstance).filter_by(id=experiment.kaapana_instance_id).first()
        )  # yes: search Kaapana instance in db according to given kaapana_instance_id

    if (
        db.query(models.Experiment).filter_by(experiment_name=experiment.experiment_name).first()
...
def get_experiments(
    ...
    if instance_name is not None:
        ...
        return (
            db.query(models.Experiment)
            .filter(models.Experiment.involved_kaapana_instances.contains(involved_instance_name))
            .all()
        )
