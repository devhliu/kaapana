...
async def create_remote_kaapana_instance(
    remote_kaapana_instance: schemas.RemoteKaapanaInstanceCreate, db: Session = Depends(get_db)
):
...
async def create_client_kaapana_instance(
    client_kaapana_instance: schemas.ClientKaapanaInstanceCreate, db: Session = Depends(get_db)
):
...
async def get_remote_kaapana_instances(
    filter_kaapana_instances: schemas.FilterKaapanaInstances = None, db: Session = Depends(get_db)
):
...
@router.get(
    "/jobs", response_model=List[schemas.JobWithKaapanaInstance]
)  # changed JobWithKaapanaInstance to JobWithExperiment
async def get_jobs(
    instance_name: str = None, status: str = None, limit: int = None, db: Session = Depends(get_db)
):
...
async def ui_form_schemas(
    ...
    if dag_id not in dags:
        raise HTTPException(
            status_code=404, detail=f"Dag {dag_id} is not part of the allowed dags, please add it!"
        )
    dag = dags[dag_id]
...
async def get_cohorts(
    request: Request, instance_name: str = None, limit: int = None, db: Session = Depends(get_db)
):
...
)  # schemas.ExperimentWithKaapanaInstance
async def create_experiment(
    request: Request, json_schema_data: schemas.JsonSchemaData, db: Session = Depends(get_db)
):
        ...
        conf_data["data_form"].update(
            {
                ...
                "cohort_identifiers": [
                    identifier.identifier for identifier in db_cohort.cohort_identifiers
                ],
            }
        )
    ...
    for jobs_to_create in queued_jobs:
        ...
        if (
            db_client_kaapana.instance_name in conf_data["experiment_form"]["runner_instances"]
        ):  # add client instance to instance list only if it is marked as an involved_instance of current experiment
    ...
    kaapana_instances_names = [
        db_kaapana_instance.instance_name for db_kaapana_instance in db_kaapana_instances_set
    ]
...
# get_experiment
@router.get(
    "/experiment", response_model=schemas.ExperimentWithKaapanaInstance
)  # response_model=schemas.ExperimentWithKaapanaInstance
async def get_experiment(
    experiment_id: int = None, experiment_name: str = None, db: Session = Depends(get_db)
):
...
# get_experiments
@router.get(
    "/experiments", response_model=List[schemas.ExperimentWithKaapanaInstance]
)  # also okay: response_model=List[schemas.Experiment]
...
@router.get(
    "/experiment_jobs", response_model=List[schemas.JobWithKaapanaInstance]
)  # changed JobWithKaapanaInstance to JobWithExperiment
