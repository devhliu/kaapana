...
async def ui_form_schemas(
    ...
    if (
        len(filter_kaapana_instances.instance_names) == 0 and filter_kaapana_instances.remote is False
...
async def ui_form_schemas(
    request: Request, filter_kaapana_instances: schemas.FilterKaapanaInstances = None, db: Session = Depends(get_db)
):
...
async def get_cohorts(request: Request, instance_name: str = None, limit: int = None, db: Session = Depends(get_db)):
    return crud.get_cohorts(
        db, instance_name, limit=limit, as_list=False, username=request.headers["x-forwarded-preferred-username"]
    )
...
async def create_experiment(request: Request, json_schema_data: schemas.JsonSchemaData, db: Session = Depends(get_db)):
    ...
    if single_execution is True:
        for cohort_identifier in data_form["cohort_identifiers"][:cohort_limit]:
            ...
            queued_jobs.append(
                {"conf_data": single_conf_data, "dag_id": json_schema_data.dag_id, "username": username}
            )
    else:
        queued_jobs = [
            {
...
async def get_experiment_jobs(
    experiment_name: str = None, status: str = None, limit: int = None, db: Session = Depends(get_db)
):
    ...
    if experiment.experiment_status == "abort":
        ...
        for db_job in db_experiment.experiment_jobs:
            ...
            job = schemas.JobUpdate(
                **{"job_id": db_job.id, "status": "abort", "description": "The job was aborted by the user!"}
            )
