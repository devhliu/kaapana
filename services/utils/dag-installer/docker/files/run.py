            ...
            if matches:
                for match in matches:
                    docker_registry_url = (
                        ...
                        else match[1].replace(default_registry_identifier, KAAPANA_DEFAULT_REGISTRY)
                    )
                    docker_image = match[3]
                    docker_version = (
                        match[4]
                        if default_version_identifier not in match[4]
                        else match[4].replace(default_version_identifier, KAAPANA_BUILD_VERSION)
                    )
...
for file_path in files_to_copy:
    ...
    if (
        not dest_path.startswith("/plugins")
        and not dest_path.startswith("/dags")
        and not dest_path.startswith("/mounted_scripts")
    ):
