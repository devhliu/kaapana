...
def requests_retry_session(
    retries=16, backoff_factor=1, status_forcelist=[404, 429, 500, 502, 503, 504], session=None, use_proxies=False
):
...
def minio_rmtree(minioClient, bucket_name, object_name):
    delete_object_list = map(
        lambda x: DeleteObject(x.object_name), minioClient.list_objects(bucket_name, object_name, recursive=True)
    )
...
class KaapanaFederatedTrainingBase(ABC):
    ...
    def __init__(
        ...
        if self.use_minio_mount is None:
            self.fl_working_dir = os.path.join(
                "/", self.workflow_dir, os.getenv("OPERATOR_OUT_DIR", "federated-operator")
            )
        else:
            self.fl_working_dir = os.path.join(
        ...
        with requests.Session() as s:
            r = requests_retry_session(session=s).post(
                f"{self.client_url}/get-remote-kaapana-instances", json={"instance_names": instance_names}
            )
        KaapanaFederatedTrainingBase.raise_kaapana_connection_error(r)
        self.remote_sites = r.json()

        self.minioClient = Minio(
            minio_host + ":" + minio_port, access_key=access_key, secret_key=secret_key, secure=False
        )
    ...
    def distribute_jobs(self, federated_round):
        ...
        with requests.Session() as s:
            r = requests_retry_session(session=s).put(
                f"{self.client_url}/experiment_jobs",
                json={
    ...
    def download_minio_objects_to_workflow_dir(self, federated_round, tmp_central_site_info):
        ...
        if federated_round > 0:
            previous_federated_round_dir = os.path.join(
                self.remote_conf_data["federated_form"]["federated_dir"], str(federated_round - 1)
            )
        else:
            previous_federated_round_dir = None
        current_federated_round_dir = os.path.join(
            self.remote_conf_data["federated_form"]["federated_dir"], str(federated_round)
        )
        ...
        for instance_name, tmp_site_info in tmp_central_site_info.items():
            ...
            objects = self.minioClient.list_objects(
                federated_bucket, os.path.join(current_federated_round_dir, instance_name), recursive=True
            )
            for obj in objects:
                ...
                if (
                    ...
                    file_path = os.path.join(
                        self.fl_working_dir,
                        os.path.relpath(obj.object_name, self.remote_conf_data["federated_form"]["federated_dir"]),
                    )
            ...
            if previous_federated_round_dir is not None and self.use_minio_mount is None:
                minio_rmtree(
                    self.minioClient, federated_bucket, os.path.join(previous_federated_round_dir, instance_name)
                )
            ...
            for file_path, next_object_name in zip(tmp_site_info["file_paths"], tmp_site_info["next_object_names"]):
                ...
                if self.use_minio_mount is None:
                    self.minioClient.fput_object(
                        self.remote_conf_data["federated_form"]["federated_bucket"], next_object_name, file_path
                    )
                else:
                    dst = os.path.join(
                        self.use_minio_mount,
    ...
    def train(self):
        for federated_round in range(
            self.federated_round_start, self.remote_conf_data["federated_form"]["federated_total_rounds"]
        ):
