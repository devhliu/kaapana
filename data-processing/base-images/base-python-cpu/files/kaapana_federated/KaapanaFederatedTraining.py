...
class KaapanaFederatedTrainingBase(ABC):
    ...
    def __init__(
        ...
        with requests.Session() as s:
            r = requests_retry_session(session=s).post(
                f"{self.client_url}/get-remote-kaapana-instances",
    ...
    def create_recovery_conf(self, federated_round):
        ...
        self.tmp_federated_site_info = {
            instance_name: {
                k: tmp_site_info[k] for k in ["from_previous_dag_run", "before_previous_dag_run"]
            }
            for instance_name, tmp_site_info in self.tmp_federated_site_info.items()
        }
        recovery_conf = {
            ...
            "instance_names": [
                instance_name for instance_name, _ in self.tmp_federated_site_info.items()
            ],
            "form_data": {
                **self.local_conf_data,
