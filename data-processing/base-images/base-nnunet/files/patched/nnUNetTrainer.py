...
from nnunet.training.dataloading.dataset_loading import load_dataset, \
    DataLoader3D, DataLoader2D, unpack_dataset
...
class nnUNetTrainer(NetworkTrainer):
    ...
    def initialize(self, training=True, force_load_plans=False):
        ...
        if training:
            ...
            self.tr_gen, self.val_gen = get_default_augmentation(
                self.dl_tr, self.dl_val, self.data_aug_params["patch_size_for_spatialtransform"], self.data_aug_params
            )
    ...
    def plot_network_architecture(self):
        try:
            ...
            if torch.cuda.is_available():
                g = hl.build_graph(
                    self.network, torch.rand((1, self.num_input_channels, *self.patch_size)).cuda(), transforms=None
                )
            else:
                g = hl.build_graph(
                    self.network, torch.rand((1, self.num_input_channels, *self.patch_size)), transforms=None
                )
    ...
    def preprocess_patient(self, input_files):
        ...
        preprocessor_class = recursive_find_python_class(
            [join(nnunet.__path__[0], "preprocessing")], preprocessor_name, current_module="nnunet.preprocessing"
        )
        assert preprocessor_class is not None, (
            "Could not find preprocessor %s in nnunet.preprocessing" % preprocessor_name
        )
        preprocessor = preprocessor_class(
            self.normalization_schemes, self.use_mask_for_norm, self.transpose_forward, self.intensity_properties
        )
    ...
    def validate(
        ...
        for k in self.dataset_val.keys():
            ...
            pred_gt_tuples.append(
                [join(output_folder, fname + ".nii.gz"), join(self.gt_niftis_folder, fname + ".nii.gz")]
            )
    ...
    def finish_online_evaluation(self):
        ...
        log_dict = {"loss/train": float(self.all_tr_losses[-1]), "loss/val": float(self.all_val_losses[-1])}
        ...
        log_dict.update({"epoch": self.epoch, "fold": self.fold, "timestamp": time.time()})
