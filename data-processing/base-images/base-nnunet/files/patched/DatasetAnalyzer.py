...
class DatasetAnalyzer(object):
    def __init__(
        self, folder_with_cropped_data, overwrite=True, num_processes=default_num_threads
    ):
        """
    ...
    def _get_voxels_in_foreground(self, patient_identifier, modality_id):
        all_data = np.load(join(self.folder_with_cropped_data, patient_identifier) + ".npz")[
            "data"
        ]
    ...
    def analyze_dataset(self, collect_intensityproperties=True):
        ...
        save_pickle(
            dataset_properties, join(self.folder_with_cropped_data, "dataset_properties.pkl")
        )
