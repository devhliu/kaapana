...
def train_val_datasplit():
    ...
    for i in range(len(imagedata_fnames)):
        ...
        unique_dcm_name = os.path.join(
            dcm_f.PatientID.split(".")[0], dcm_f.StudyInstanceUID, dcm_f.SeriesInstanceUID, "000000.dcm"
        )
        index_in_metadata = (
            [i for i, s in enumerate(metadata_all_t[11]) if unique_dcm_name in s][0]
            if len([i for i, s in enumerate(metadata_all_t[11]) if unique_dcm_name in s]) != 0
            else None
        )
        index_in_metadata = (
            [i for i, s in enumerate(metadata_all_t[12]) if unique_dcm_name in s][0]
            if len([i for i, s in enumerate(metadata_all_t[12]) if unique_dcm_name in s]) != 0
            else index_in_metadata
        )
        index_in_metadata = (
            [i for i, s in enumerate(metadata_all_t[13]) if unique_dcm_name in s][0]
            if len([i for i, s in enumerate(metadata_all_t[13]) if unique_dcm_name in s]) != 0
            else index_in_metadata
        )
