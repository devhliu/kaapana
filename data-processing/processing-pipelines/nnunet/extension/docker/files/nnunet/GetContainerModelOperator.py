...
class GetContainerModelOperator(KaapanaBaseOperator):
    ...
    def pre_execute(self, context):
        ...
        if (
            ...
            and "task" in context["dag_run"].conf["form_data"]
        ):
    ...
    def execute(self, context):
        ...
        if not self.skip_execute:
            self.image = (
                f"{self.registry_url}/model-nnunet-{self.task_id.lower().replace('_', '-')}:{self.model_version}"
            )
    ...
    def __init__(
        ...
        **kwargs,
        ...
        volume_mounts.append(VolumeMount("models", mount_path="/models_mount", sub_path=None, read_only=False))
        volume_config = {"persistentVolumeClaim": {"claimName": "models-jobs-pv-claim"}}
