...
class LocalDiceOperator(KaapanaPythonBaseOperator):
    def create_plots(self, data_table, table_name, result_dir):
        ...
        box_plot = sns.boxplot(
            x="Model", y="Dice", hue="Label", palette="Set3", data=data_table, ax=ax1, order=plot_labels
        )
        ...
        for batch_element_dir in batch_folders:
            ...
            for single_model_pred_file in single_model_pred_files:
                ...
                one_hot_encoding_pred = (
                    (np.arange(max_label + 1) == single_model_prediction[..., None]).astype(int).transpose()
                )
    ...
    def __init__(
        self, dag, gt_operator, ensemble_operator=None, batch_name=None, workflow_dir=None, anonymize=True, **kwargs
    ):
