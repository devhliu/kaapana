    ...
    def get_json(self, filename, json_list):
        if len(json_list) == 1:
            ...
            ensemble_file_filtered = [
                json_file for json_file in json_list if "ensemble_seg_info.json" in json_file
            ]
            ...
            for nifti_file in nifti_files:
                if self.target == "batch":
                    ...
                    target_batch_element = self.get_batch_element(
                        filename=nifti_file, batch_path=join(run_dir, self.target_batchname)
                    )
                ...
                if json_file is not None:
                    target_json_path = join(
                        target_dir, basename(json_file).replace(".json", f"-{model_id}.json")
                    )
