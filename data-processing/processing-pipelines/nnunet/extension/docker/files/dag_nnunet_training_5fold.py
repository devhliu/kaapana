...
dag = DAG(
    dag_id="nnunet-training-5fold", default_args=args, concurrency=gpu_count, max_active_runs=1, schedule_interval=None
)
...
get_ref_ct_series_from_seg = LocalGetRefSeriesOperator(
    dag=dag, input_operator=get_input, search_policy="reference_uid", parallel_downloads=5, modality=None
)
dcm2nifti_ct = DcmConverterOperator(
    dag=dag, input_operator=get_ref_ct_series_from_seg, parallel_id="ct", output_format="nii.gz"
)
...
check_seg = LocalSegCheckOperator(
    dag=dag, abort_on_error=True, move_data=False, input_operators=[dcm2nifti_seg, dcm2nifti_ct]
)
...
bin2dcm = Bin2DcmOperator(dag=dag, input_operator=nnunet_export, study_uid=study_uid, file_extensions="*.zip")

dcmseg_send = DcmSendOperator(dag=dag, level="batch", ae_title="nnunet-models", input_operator=bin2dcm)
