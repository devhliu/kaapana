...
def get_seg_info(input_nifti):
    ...
    if len(json_files_found) > 0 and "-meta.json" in json_files_found[0]:
        ...
        json_file_found = [
            list_json for list_json in json_files_found if seg_nifti_id.split("--")[0] in list_json
        ]
        ...
        filtered_jsons = [
            meta_json_path for meta_json_path in json_files_found if seg_nifti_id in meta_json_path
        ]
...
def resample_image(input_path, original_path, replace=True, target_dir=None):
    ...
    output = run(
        command, stdout=PIPE, stderr=PIPE, universal_newlines=True, timeout=execution_timeout
    )
...
for batch_element_dir in batch_folders:
    ...
    for seg_file in seg_files:
        if (
            seg_file
            not in base_image_ref_dict[base_series_id]["batch_elements"][batch_element_dir]
        ):
            base_image_ref_dict[base_series_id]["batch_elements"][batch_element_dir][
                "seg_files"
            ].append(seg_file)
    ...
    for batch_element_dir in sorted(
        ...
        key=lambda batch_element_dir: base_image_ref_dict[base_series_id]["batch_elements"][
            batch_element_dir
        ]["file_count"],
        reverse=True,
    ):
