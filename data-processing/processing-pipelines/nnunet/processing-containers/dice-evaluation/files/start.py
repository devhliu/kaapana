...
def get_seg_info(input_nifti):
    ...
    if "seg-check-inference" in input_nifti:
        model_id_info_file = join(
            dirname(input_nifti).replace("seg-check-inference", "do-inference"), f"seg_info{model_id}.json"
        )
...
def get_metric_score(input_data):
    ...
    for model_pred_file in single_model_pred_files:
        ...
        dice_scores = compute_meandice(y_pred=pred_tensor, y=gt_tensor, include_background=include_background).numpy()[
            0
        ]
        ...
        results[model_id] = {
            pred_file_id: {"dice_scores": list(dice_scores), "asd_scores": list(np.float32(asd_scores))}
    ...
    if ensemble_pred_file is not None:
        ...
        results["ensemble"] = {
            ensemble_file_id: {"dice_scores": list(dice_scores), "asd_scores": list(np.float32(asd_scores))}
        }
    ...
    for model_id, file_results in model_results.items():
        ...
        for file_id, dice_info in file_results.items():
            ...
            for array_index in range(0, len(dice_info["dice_scores"])):
                ...
                result_table.append([file_id, model_id, class_label, class_dice, class_asd])
