...
dag = DAG(
    dag_id="service-parse-doccano-tokens", default_args=args, schedule_interval=None, concurrency=10, max_active_runs=5
)

download_dataset = LocalDoccanoDownloadDatasetOperator(dag=dag)
unzip_files = ZipUnzipOperator(dag=dag, input_operator=download_dataset, mode="unzip", batch_level=True)

doccano_tags = ProcessStudyIdsOperator(dag=dag, input_operator=unzip_files)

tag_dataset = LocalTaggingOperator(
    dag=dag, input_operator=doccano_tags, add_tags_from_file=True, tags_to_add_from_file=["doccano_tags"]
)
