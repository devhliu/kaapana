...
class MitkInputOperator(KaapanaPythonBaseOperator):
            ...
            if len(dcm_files) > 0:
                ...
                if "ReferencedSeriesSequence" in incoming_dcm:
                    ...
                    result = self.downloadSeries(
                        studyUID=incoming_dcm.StudyInstanceUID, seriesUID=seriesUID, target_dir=target_dir
                    )
                    if result:
    ...
    def __init__(
        ...
        **kwargs
        ...
        super().__init__(
            dag=dag, name="get-mitk-input", operator_out_dir=operator_out_dir, python_callable=self.get_files, **kwargs
        )
