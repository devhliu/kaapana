...
class nnUNetFederatedTraining(KaapanaFederatedTrainingBase):
    ...
    def update_data(self, federated_round, tmp_central_site_info):
        ...
        if federated_round == -2:
                    ...
                    if idx == 0:
                        ...
                        if "intensityproperties" in concat_dataset_properties:
                            ...
                            for mod_id, _ in concat_dataset_properties["intensityproperties"].items():
                                voxels_in_foreground[mod_id] = concat_dataset_properties["intensityproperties"][
                                    mod_id
                                ]["v"]
                                local_intensityproperties[mod_id] = collections.OrderedDict()
                                local_intensityproperties[mod_id]["local_props"] = concat_dataset_properties[
                                    "intensityproperties"
                                ][mod_id]["local_props"]
                                print(idx, mod_id)
                        ...
                        if "intensityproperties" in concat_dataset_properties:
                            for mod_id, intensityproperties in concat_dataset_properties[
                                "intensityproperties"
                            ].items():
                                voxels_in_foreground[mod_id] = (
                                    voxels_in_foreground[mod_id]
                                    + dataset_properties["intensityproperties"][mod_id]["v"]
                                )
                                concat_dataset_properties["intensityproperties"][mod_id]["local_props"].update(
                                    dataset_properties["intensityproperties"][mod_id]["local_props"]
                                )
                                print(idx, mod_id)
            ...
            if (
                self.remote_conf_data["federated_form"]["federated_total_rounds"] == federated_round + 1
                ...
                shutil.copyfile(
                    src=os.path.join(os.path.dirname(fname), "dataset.json"), dst=os.path.join(dst, "dataset.json")
    ...
    def on_wait_for_jobs_end(self, federated_round):
        if federated_round == -2:
            ...
            self.remote_conf_data["federated_form"]["skip_operators"] = self.remote_conf_data["federated_form"][
                "skip_operators"
            ] + ["get-input-data", "get-ref-series-ct", "dcmseg2nrrd", "dcm-converter-ct", "seg-check"]
