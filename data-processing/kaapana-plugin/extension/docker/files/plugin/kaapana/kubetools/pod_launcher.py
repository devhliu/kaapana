...
class PodLauncher(LoggingMixin):
    ...
    def __init__(
        self, kube_client=None, in_cluster=True, cluster_context=None, extract_xcom=False
    ):
        super(PodLauncher, self).__init__()
        self._client, self._batch_client, self._extensions_client = kube_client or get_kube_client(
            ...
            while self.pod_not_started(pod):
                if last_status != pod.last_kube_status:
                    ...
                    if (
                        ...
                        self.log.debug(
                            "Pod has status {} -> waiting...!".format(pod.last_kube_status)
                        )
                        pull_image = True
                ...
                if delta.seconds >= startup_timeout:
                    self.log.exception(
                        "Pod took too long to start! startup_timeout: {}".format(startup_timeout)
                    )
    ...
    def base_container_is_running(self, pod):
        ...
        status = next(
            iter(filter(lambda s: s.name == "base", event.status.container_statuses)), None
        )
    ...
    def process_status(self, event, pod):
        ...
        if event.status.container_statuses is not None:
            ...
            if (
                ...
                if pod.last_kube_status != "UNSCHEDULABLE":
                    self.log.warning(
                        "Insufficient quota: {}".format(event.status.conditions[0].message)
                    )
