...
def dag_run_status(dag_id, run_id):
    ...
    time_format = "%Y-%m-%dT%H:%M:%S"
    return jsonify(
        dag_id=dag_id, run_id=run_id, state=dag_run.state, execution_date=dag_run.execution_date.strftime(time_format)
    )
...
def get_os_dashboards():
    try:
        res = HelperOpensearch.os_client.search(
            body={"query": {"exists": {"field": "dashboard"}}, "_source": ["dashboard.title"]}, size=10000, from_=0
        )
...
def get_static_website_results():
    ...
    def build_tree(item, filepath, org_filepath):
        ...
        if len(splits) == 1:
            ...
            item["vuetifyFiles"].append(
                {"name": splits[0], "file": "html", "path": f"/static-website-browser/{org_filepath}"}
            )
        else:
    ...
    def get_vuetify_tree_structure(tree):
        ...
        for parent, children in tree.items():
            ...
            if parent == "vuetifyFiles":
                ...
                subtree.append(
                    {"name": parent, "path": str(uuid.uuid4()), "children": get_vuetify_tree_structure(children)}
                )
        return subtree
    ...
    _minio_port = "9000"
    minioClient = Minio(
        _minio_host + ":" + _minio_port, access_key="kaapanaminio", secret_key="Kaapana2020", secure=False
    )
...
def tag_data():
    ...
    def tagging(series_instance_uid: str, tags: List[str], tags2add: List[str] = [], tags2delete: List[str] = []):
    ...
    try:
        data = request.get_json(force=True)
        for series in data:
            tagging(series["series_instance_uid"], series["tags"], series["tags2add"], series["tags2delete"])
        return jsonify({}), 200
...
def get_query_elastic_structured():
    ...
    try:
        res = HelperOpensearch.os_client.search(
            body={
                ...
                "aggs": {
                    "Patients": {
                        "terms": {"field": "00100020 PatientID_keyword.keyword", "size": 10000},
                        "aggs": {
                            "Modalities": {"terms": {"field": "00080060 Modality_keyword.keyword"}},
                            "Studies": {
                                "terms": {"field": "0020000D StudyInstanceUID_keyword.keyword", "size": 10000},
                                "aggs": {
                                    "Series": {
                                        "top_hits": {
                                            "size": 100,
                                            ...
                                            "sort": [{"00080020 StudyDate_date": {"order": "desc"}}],
                                        }
                                    }
                                },
                            },
    ...
    for patient in res["aggregations"]["Patients"]["buckets"]:
        ...
        for study in patient["Studies"]["buckets"]:
            study_dict = dict(study_key=study["key"], series_count=study["doc_count"], series=[])
            for i, series in enumerate(study["Series"]["hits"]["hits"]):
                ...
                if i == 0:
                    ...
                    if "00101010 PatientAge_integer" in series["_source"].keys():
                        patient_dict["00101010 PatientAge_integer"] = series["_source"]["00101010 PatientAge_integer"]
                    if "00100010 PatientName_keyword_alphabetic" in series["_source"].keys():
                        patient_dict["00100010 PatientName_keyword_alphabetic"] = series["_source"][
                            "00100010 PatientName_keyword_alphabetic"
                        ]
                    if "00100040 PatientSex_keyword" in series["_source"].keys():
                        patient_dict["00100040 PatientSex_keyword"] = series["_source"]["00100040 PatientSex_keyword"]
                    # Study params
                    if "00081030 StudyDescription_keyword" in series["_source"].keys():
                        study_dict["00081030 StudyDescription_keyword"] = series["_source"][
                            "00081030 StudyDescription_keyword"
                        ]
                    if "00080020 StudyDate_date" in series["_source"].keys():
                        study_dict["00080020 StudyDate_date"] = series["_source"]["00080020 StudyDate_date"]
...
def format_metadata():
    ...
    for d_ in parsed_and_filtered_data:
        ...
        res.append(dict(name=d_.description, value=str(value)))
...
@kaapanaApi.route("/api/curation_tool/seriesInstanceUID/<string:seriesInstanceUID>")
@csrf.exempt
def get_data_for_series_instance_UID(seriesInstanceUID):
    try:
        res = HelperOpensearch.os_client.search(body={"query": {"ids": {"values": [seriesInstanceUID]}}})
    except Exception as e:
        print("ERROR in elasticsearch search!")
        return jsonify({"Error message": e}), 500
...
def get_query_values():
    ...
    def camel_case_to_space(s):
        return " ".join(
            re.sub(
                "([A-Z][a-z]+)",
                r" \1",
                re.sub("([A-Z]+)", r" \1", " ".join(" ".join(s.split(" ")[-1::]).split("_")[:-1])),
            ).split()
        )
    ...
    try:
        ...
        name_field_map = {
            k: v for k, v in name_field_map.items() if len(re.findall("\d", k)) == 0 and k != "" and v != ""
        }
        ...
        for name, field in name_field_map.items():
            temp_res = HelperOpensearch.os_client.search(
                body={"size": 0, "query": query, "aggs": {name: {"terms": {"field": field, "size": 10000}}}}
            )
