...
def get_dag_runs(dag_id):
    ...
    dag_runs = (
        session.query(DagRun).filter(DagRun.dag_id == dag_id).order_by(DagRun.execution_date).all()
    )
...
def dag_run_status(dag_id, run_id):
    ...
    try:
        ...
        return Response(
            "RunId {} does not exist for Dag {}".format(run_id, dag_id), HTTPStatus.BAD_REQUEST
        )
...
def get_minio_credentials():
    ...
    return (
        jsonify({"accessKey": access_key, "secretKey": secret_key, "sessionToken": session_token}),
        200,
    )
...
def get_os_dashboards():
    try:
        res = HelperOpensearch.os_client.search(
            body={"query": {"exists": {"field": "dashboard"}}, "_source": ["dashboard.title"]},
            size=10000,
            from_=0,
        )
...
def tag_data():
    ...
    def tagging(
        ...
        final_tags = list(
            set(tags).union(set(index_tags)).difference(set(tags2delete)).union(set(tags2add))
        )
                                ...
                                "aggs": {
                                    "Series": {
                                        "top_hits": {
                                            ...
                                            "sort": [
                                                {"00080020 StudyDate_date": {"order": "desc"}}
                                            ],
                                        }
                                    }
                                },
            ...
            for i, series in enumerate(study["Series"]["hits"]["hits"]):
                ...
                if i == 0:
                    ...
                    if "00100010 PatientName_keyword_alphabetic" in series["_source"].keys():
                        patient_dict["00100010 PatientName_keyword_alphabetic"] = series[
                            "_source"
                        ]["00100010 PatientName_keyword_alphabetic"]
...
def get_data_for_series_instance_UID(seriesInstanceUID):
    try:
        res = HelperOpensearch.os_client.search(
            body={"query": {"ids": {"values": [seriesInstanceUID]}}}
        )
    except Exception as e:
        print("ERROR in elasticsearch search!")
        return jsonify({"Error message": e}), 500
...
def get_query_values():
    ...
    try:
        res = HelperOpensearch.os_client.indices.get_mapping("meta-index")["meta-index"][
            "mappings"
        ]["properties"]
