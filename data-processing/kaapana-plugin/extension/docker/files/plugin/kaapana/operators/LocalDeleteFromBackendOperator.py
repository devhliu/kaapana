    ...
    * Input data which should be removed is given via input parameter: delete_operator.
    ...
    def start(self, ds, **kwargs):
        ...
        for batch_element_dir in batch_folder:
            dcm_files = sorted(
                [*Path(batch_element_dir, self.operator_in_dir).rglob("*.dcm*")]
            )
            if len(dcm_files) > 0:
                ...
                if resp.status_code == 404:
                    print(
                        "Item not found in database. This might happen when an item was not used in a Dataset/Cohort"
                    )
                elif resp.status_code != 200:
                    print(
                        f"Failed to delete {series_uid} from backend with: [{resp.status_code}] {resp.text}"
                    )
                    exit(1)
            else:
                json_files = sorted(
                    [*Path(batch_element_dir, self.operator_in_dir).rglob("*.json*")]
                )
    ...
    def __init__(self, dag, delete_operator=None, **kwargs):
        """
        :param delete_operator:
        """

        super().__init__(
            dag=dag, name="delete-backend", python_callable=self.start, **kwargs
        )
