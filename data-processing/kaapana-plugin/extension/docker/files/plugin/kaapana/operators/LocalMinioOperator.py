...
class LocalMinioOperator(KaapanaPythonBaseOperator):
    ...
    def start(self, ds, **kwargs):
        ...
        if conf is not None and "Key" in conf:
            ...
            print(f"Setting bucket name to {self.bucket_name} " f"and object name to {self.object_name}")
        ...
        run_dir = (
            os.path.join(WORKFLOW_DIR, kwargs["dag_run"].run_id)
        ...
        for batch_element_dir in batch_folder:
            ...
            for action_operator in self.action_operators:
                object_dirs.append(
                    os.path.relpath(os.path.join(batch_element_dir, action_operator.operator_out_dir), run_dir)
                )
        ...
        if self.zip_files:
            ...
            HelperMinio.apply_action_to_file(
                minio_client, "put", self.bucket_name, zip_object_name, zip_file_path, self.file_white_tuples
            )
            return
        ...
        if object_names:
            ...
            HelperMinio.apply_action_to_object_names(
                minio_client, self.action, self.bucket_name, run_dir, object_names, self.file_white_tuples
            )
        else:
            if not object_dirs:
    ...
    def __init__(
        ...
        **kwargs,
        ...
        super(LocalMinioOperator, self).__init__(
            dag=dag, name=name, python_callable=self.start, execution_timeout=timedelta(minutes=30), **kwargs
        )
