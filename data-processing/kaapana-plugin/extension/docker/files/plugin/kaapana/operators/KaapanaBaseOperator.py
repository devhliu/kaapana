...
class KaapanaBaseOperator(BaseOperator, SkipMixin):
    ...
    def __init__(
        ...
        image_pull_policy=env_pull_policy
        if env_pull_policy == env_pull_policy.lower() == "never"
        else "IfNotPresent",
        ...
        **kwargs,
        ...
        self.volume_mounts.append(
            VolumeMount(
                "mounted-scripts",
                mount_path="/app/mounted_scripts",
                sub_path=None,
                read_only=False,
            )
        )
    ...
    def execute(self, context):
        config_path = os.path.join(
            self.workflow_dir, context["run_id"], "conf", "conf.json"
        )
        ...
        self.kube_name = cure_invalid_name(
            context["run_id"].replace(
                context["dag_run"].dag_id, context["task_instance"].task_id
            ),
        ...
        self.env_vars.update(
            {"RUN_ID": context["dag_run"].run_id, "DAG_ID": context["dag_run"].dag_id}
        )
        self.env_vars[
            "WORKFLOW_DIR"
        ] = f"{self.env_vars['WORKFLOW_DIR']}/{context['run_id']}"
        ...
        if self.dev_server is not None:
            ...
            for idx, (k, v) in enumerate(self.env_vars.items()):
                env_vars_sets.update(
                    {f"envVars[{idx}].name": f"{k}", f"envVars[{idx}].value": f"{v}"}
                )
            ...
            for volume_mount in self.volume_mounts:
                if (
                    volume_mount.name in volume_mounts_sets.values()
        ...
        if self.delete_output_on_start is True:
            KaapanaBaseOperator.delete_operator_out_dir(
                context["run_id"], self.operator_out_dir
            )
            ...
            if launcher_return is None:
                ...
                if result != State.SUCCESS:
                    raise AirflowException(
                        "Pod returned a failure: {state}".format(state=message)
                    )
                else:
