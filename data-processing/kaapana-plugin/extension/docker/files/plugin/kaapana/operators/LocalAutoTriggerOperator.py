...
class LocalAutoTriggerOperator(KaapanaPythonBaseOperator):
                ...
                if dag.dag_id == dag_id:
                    ...
                    for task in dag.tasks:
                        if "LocalGetInputDataOperator" == task.__class__.__name__:
                            ...
                            target = os.path.join(
                                "/data", dag_run_id, "batch", series_uid, get_input_dir_name
                            )
    ...
    def start(self, ds, **kwargs):
        ...
        batch_folders = sorted(
            [f for f in glob(join(WORKFLOW_DIR, kwargs["dag_run"].run_id, "batch", "*"))]
        )
        triggering_list = []
                    ...
                    for (
                        ...
                        if dag_id == "service-extract-metadata" or (
                            ...
                            triggering_list.append(
                                {"dag_id": dag_id, "conf": conf, "dag_run_id": dag_run_id}
                            )
