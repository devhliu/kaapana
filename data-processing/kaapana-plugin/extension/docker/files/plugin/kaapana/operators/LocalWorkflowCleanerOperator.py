...
class LocalWorkflowCleanerOperator(KaapanaPythonBaseOperator):
    ...
    def start(self, ds, **kwargs):
        ...
        if conf is not None and "client_job_id" in conf:
            update_job(
                conf["client_job_id"], status="finished", run_id=dag_run.run_id, description=f"Finished successfully"
            )

    def __init__(self, dag, run_dir: str = None, clean_workflow_dir: bool = True, **kwargs):
        """
        ...
        super().__init__(dag=dag, name="workflow-cleaner", python_callable=self.start, **kwargs)
