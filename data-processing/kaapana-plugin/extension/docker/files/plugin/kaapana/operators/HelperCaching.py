...
def update_job(client_job_id, status, run_id=None, description=None):
    with requests.Session() as s:
        r = requests_retry_session(session=s).get(
            JOB_API_URL, timeout=TIMEOUT, params={"job_id": client_job_id}
        )
    ...
    with requests.Session() as s:
        r = requests_retry_session(session=s).put(
            ...
            json={
                ...
                "owner_kaapana_instance_name": client_job[
...
def from_previous_dag_run_action(operator_out_dir, action, dag_run_dir, federated):
    if action == "from_previous_dag_run":
        src = os.path.join(
            WORKFLOW_DIR, federated["from_previous_dag_run"], operator_out_dir
        )
        print(src)
    ...
    if action == "from_previous_dag_run":
        src_root_dir = os.path.join(
            WORKFLOW_DIR, federated["from_previous_dag_run"], BATCH_NAME
        )
    ...
    def wrapper(self, *args, **kwargs):
        ...
        if conf is not None and "client_job_id" in conf:
            trying_request_action(
                ...
                status="running",
                run_id=run_id,
                description=f"Running the operator {self.name}",
            )
        ...
        try:
            ...
            if conf is not None and "client_job_id" in conf:
                trying_request_action(
                    ...
                    status="failed",
                    run_id=run_id,
                    description=f"Operator {self.name} had an exception {e}",
                )
            raise e
        ...
        if (
            ...
            if conf is not None and "client_job_id" in conf:
                trying_request_action(
                    ...
                    description=f"Finished successfully",
                )
