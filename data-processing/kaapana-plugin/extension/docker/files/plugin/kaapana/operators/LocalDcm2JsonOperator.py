...
class LocalDcm2JsonOperator(KaapanaPythonBaseOperator):
    ...
    * json file: output json file. DICOM tags are converted to a json file.
    ...
    def executeDcm2Json(self, inputDcm, outputJson):
        ...
        command = self.dcm2json_path + " " + self.withAppostroph(inputDcm) + " " + self.withAppostroph(outputJson)
    ...
    def cleanJsonData(self, path):
        ...
        if "00080060" in dicom_metadata and (
            dicom_metadata["00080060"]["Value"][0] == "RTSTRUCT" or dicom_metadata["00080060"]["Value"][0] == "SEG"
        ):
    ...
    def __init__(self, dag, exit_on_error=False, delete_pixel_data=True, bulk=False, **kwargs):
        ...
        super().__init__(dag=dag, name="dcm2json", python_callable=self.start, ram_mem_mb=10, **kwargs)
