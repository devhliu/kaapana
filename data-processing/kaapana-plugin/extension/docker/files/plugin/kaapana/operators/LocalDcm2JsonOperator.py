    ...
    * json file: output json file. DICOM tags are converted to a json file.
    ...
    def get_label_tags(metadata):
        ...
        if "30060080" in metadata:
            try:
                ref_list = [
                    entry["300600A4"]["Value"][0] for entry in metadata["30060080"]["Value"]
                ]
        ...
        result_dict["segmentation_labels_list_keyword"] = (
            ",".join(sorted(segmentation_label_list)) if len(segmentation_label_list) > 0 else None
        )
        result_dict["00620005 Segment Label_keyword"] = segmentation_label_list
        result_dict["rtstruct_organ_list_keyword"] = (
            ",".join(sorted(rtstruct_organ_list)) if len(rtstruct_organ_list) > 0 else None
        )
        result_dict["rtstruct_marker_list_keyword"] = (
            ",".join(sorted(rtstruct_marker_list)) if len(rtstruct_marker_list) > 0 else None
        )
        result_dict["rtstruct_other_list_keyword"] = (
            ",".join(sorted(rtstruct_other_list)) if len(rtstruct_other_list) > 0 else None
        )
    ...
    def start(self, ds, **kwargs):
        ...
        for batch_element_dir in batch_folder:
            dcm_files = sorted(
                glob.glob(
                    os.path.join(batch_element_dir, self.operator_in_dir, "*.dcm*"), recursive=True
                )
            )
            ...
            for dcm_file_path in dcm_files:
                ...
                json_file_path = os.path.join(
                    target_dir, "{}.json".format(os.path.basename(batch_element_dir))
                )
        ...
        for key, value in list(dicom_meta.items()):
            ...
            if "vr" in value and "Value" in value:
                            ...
                            if isinstance(value_str, list):
                                ...
                                for date_str in value_str:
                                    ...
                                    date_formatted.append(
                                        parser.parse(date_str).strftime(self.format_date)
                                    )
                            else:
                ...
                if "InlineBinary" in value:
                    ...
                    print(
                        "No vr found in entry: {}".format(str(value).strip("[]").encode("utf-8"))
                    )
                else:
                    print(
                        "##########################################################################        replace_tags ELSE!"
    ...
    def cleanJsonData(self, path):
        ...
        label_results = {}
        if "00080060" in dicom_metadata and (
            dicom_metadata["00080060"]["Value"][0] == "RTSTRUCT"
            or dicom_metadata["00080060"]["Value"][0] == "SEG"
        ):
        ...
        new_meta_data["timestamp_arrived_hour_integer"] = new_meta_data[
            "timestamp_arrived_datetime"
        ][11:13]
    ...
    def __init__(self, dag, exit_on_error=False, delete_pixel_data=True, bulk=False, **kwargs):
        """
        ...
        super().__init__(
            dag=dag, name="dcm2json", python_callable=self.start, ram_mem_mb=10, **kwargs
        )
