...
from kaapana.blueprints.kaapana_utils import get_operator_properties, \
    requests_retry_session, trying_request_action
...
def apply_minio_presigned_url_action(
    action, federated, operator_out_dir, root_dir, client_job_id, whitelist_federated_learning=None
):
    ...
    with requests.Session() as s:
        r = requests_retry_session(session=s).get(
            f"{MINIO_CLIENT_URL}/remote-kaapana-instance",
            timeout=60,
        ...
        with open(filename, "rb") as tar:
            ...
            with requests.Session() as s:
                r = requests_retry_session(session=s, use_proxies=True).post(
                    ...
                    timeout=600,
                    files={"file": tar},
                    headers={"FederatedAuthorization": remote_network["token"], "presigned-url": data["path"]},
                )
                raise_kaapana_connection_error(r)
...
def federated_action(
    operator_out_dir, action, dag_run_dir, federated, client_job_id, whitelist_federated_learning=None
):
