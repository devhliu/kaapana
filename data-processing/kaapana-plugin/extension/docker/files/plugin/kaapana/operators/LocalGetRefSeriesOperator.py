...
class LocalGetRefSeriesOperator(KaapanaPythonBaseOperator):
        ...
        try:
            if self.data_type == "dicom":
                download_successful = HelperDcmWeb.downloadSeries(
                    seriesUID=series["reference_series_uid"], target_dir=series["target_dir"]
                )
    ...
    def get_files(self, ds, **kwargs):
        ...
        if self.target_level == "batch" and self.search_policy == None:
            ...
            if len(pacs_series) == 0 or (
                self.expected_file_count != "all" and len(pacs_series) != self.expected_file_count
            ):
                print(
                    "# ++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++"
                )
            ...
            for batch_element_dir in batch_folder:
                ...
                if (
                    ...
                    dcm_files = sorted(
                        glob.glob(
                            join(batch_element_dir, self.operator_in_dir, "*.dcm*"), recursive=True
                        )
                    )
                    if len(dcm_files) > 0:
                ...
                for series in pacs_series:
                    ...
                    if self.target_level == "batch":
                        target_dir = join(
                            run_dir, self.operator_out_dir, series_uid, self.operator_out_dir
                        )
                    else:
    ...
    def __init__(
        ...
        **kwargs,
        ...
        self.pacs_dcmweb = (
            pacs_dcmweb_host + ":" + pacs_dcmweb_port + "/dcm4chee-arc/aets/" + aetitle.upper()
        )
