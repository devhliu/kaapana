...
dag = DAG(dag_id="dcm-struct-extraction", default_args=args, max_active_runs=max_active_runs, schedule_interval=None)
...
get_ref_ct_series_from_struct = LocalGetRefSeriesOperator(
    dag=dag, input_operator=get_input, search_policy="reference_uid", parallel_downloads=5
)

dcmstruct2nifti = DcmStruct2Nifti(dag=dag, input_operator=get_input, dicom_operator=get_ref_ct_series_from_struct)
