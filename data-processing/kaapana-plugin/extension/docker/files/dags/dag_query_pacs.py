...
class Dcm2MetaJsonLinesOperator(KaapanaPythonBaseOperator):
    def dcm2meta_json_func(self, ds, **kwargs):
        ...
        for batch_element_dir in batch_folder:
            jsonl_files = sorted(
                glob.glob(os.path.join(batch_element_dir, self.operator_in_dir, "*.jsonl"), recursive=True)
            )
    ...
    def __init__(self, dag, ae_title, local_ae_title, pacs_host, pacs_port, **kwargs):
        super().__init__(dag=dag, name="dcm_json2meta_json", python_callable=self.dcm2meta_json_func, **kwargs)


dag = DAG(dag_id="query-pacs", default_args=args, concurrency=1, max_active_runs=10, schedule_interval=None)
