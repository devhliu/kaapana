...
get_object_from_minio = LocalMinioOperator(
    dag=dag, action_operator_dirs=["dicoms"], file_white_tuples=(".zip"), operator_out_dir="dicoms"
)

unzip_files = ZipUnzipOperator(dag=dag, input_operator=get_object_from_minio, batch_level=True, mode="unzip")
...
remove_object_from_minio = LocalMinioOperator(
    dag=dag, parallel_id="removing", action="remove", file_white_tuples=(".zip"), trigger_rule=TriggerRule.ALL_DONE
)
