...
dag = DAG(dag_id="tag-seg-ct-tuples", default_args=args, concurrency=10, max_active_runs=1, schedule_interval=None)

get_input_dicom = LocalGetInputDataOperator(dag=dag, name="get-input-dicom", check_modality=True, parallel_downloads=5)

get_input_json = LocalGetInputDataOperator(
    dag=dag, name="get-input-json", check_modality=True, data_type="json", parallel_downloads=5
)
