...
class DicomQueryClient:
    ...
    def execute_unlimited_query(self, ds: pydicom.dataset.Dataset):
        ...
        for status, identifier in responses:
            ...
            if not status:
                error_cnt += 1
                self.log.error(
                    "Connection time out, abort or invalid response (Status: %s)", code_to_category(status.Status)
                )
    ...
    def execute_limited_query(
        self, ds: pydicom.dataset.Dataset, limit: int = 1000, start_dt: datetime = None, end_dt: datetime = None
    ):
