...
class DicomQueryClient:
    ...
    def set_time_frame(
        self, ds: pydicom.dataset.Dataset, start_dt: datetime = None, end_dt: datetime = None
        ...
        while True:
            ...
            if new_dpd == 0:
                ...
                if attempts_without_data >= MAX_ATTEMPTS:
                    self.log.info(
                        "Last %d querys returned no result, aborting...", attempts_without_data
                    )
                    return
            else:
                attempts_without_data = 0
...
"""
    ...
    parser.add_argument(
        "-aet", help="application entity title of the SCU (e.g. this script)", default="STORESCP"
    )
    parser.add_argument(
        "-aec", help="application entity title of the SCP (e.g. the peer)", default="ANY-SCP"
    )
    parser.add_argument(
        "--max-query-size",
        type=int,
    ...
    parser.add_argument(
        "--filter-uid", help="remove results without uid according to level", action="store_true"
    )
