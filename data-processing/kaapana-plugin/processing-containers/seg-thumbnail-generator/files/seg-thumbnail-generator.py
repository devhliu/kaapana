...
def create_pngs(path_input_img: Path, path_input_seg: Path, path_output_dir: Path, all_slices=False):
    """
    ...
    res_img = sitk.LabelOverlay(image=img_slice_255, labelImage=seg_slice, opacity=0.3, backgroundValue=0.0)
    # Save
    output_path = os.path.join(path_output_dir, f'{path_input_seg.name.replace(".nii.gz", "")}.png')
    ...
    if all_slices:
        ...
        for sl in all_slices_with_annotations:
            ...
            res_img = sitk.LabelOverlay(image=img_slice_255, labelImage=seg_slice, opacity=0.3, backgroundValue=0.0)
            # Save
            output_path = os.path.join(path_output_dir, f'{path_input_seg.name.replace(".nii.gz", "")}_{sl}.png')
            print(f"Writing file {output_path}")
            sitk.WriteImage(res_img, output_path)
...
if __name__ == "__main__":
    ...
    for batch_element_dir in batch_folders:
        ...
        print(f"Creating a segmentation thumbnail for input " f"{orig_input_element} and seg {seg_element}.")
