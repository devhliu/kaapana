...
def create_segment_attribute(
    segment_algorithm_type, segment_algorithm_name, code_meaning, color, label_name="", labelID=1
):
    try:
...
multi_label_seg_name = os.environ.get(
...
for batch_element_dir in batch_folders:
    ...
    if input_type == "single_label_segs":
        ...
        for idx, seg_filepath in enumerate(segmentation_paths):
            ...
            code_meaning, segmentation_information["SeriesDescription"] = process_seg_info(
                single_label_seg_info, series_description
            )
            color = (
                np.round(np.array(cm.get_cmap("gist_ncar", 20)(random.randint(0, 19))[:3]) * 255)
                .astype(int)
                .tolist()
            )
        ...
        for idx, label in enumerate(label_info):
            ...
            color = (
                np.round(np.array(cm.get_cmap("gist_ncar", label_counts)(idx)[:3]) * 255)
                .astype(int)
                .tolist()
            )
            segment_attribute = create_segment_attribute(
