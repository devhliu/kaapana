    ...
    for dcm_filepath in dcm_paths:
        ...
        for idx, segment in enumerate(meta_data["segmentAttributes"]):
            ...
            if seg_filter is None or segment_label.lower().replace(",", " ").replace("  ", " ") in seg_filter:
                ...
                os.rename(
                    os.path.join(element_output_dir, f'{json_output}-{segment_info["labelID"]}.{output_type}'),
                    os.path.join(
                        element_output_dir, f'{json_output}--{segment_info["labelID"]}--{segment_label}.{output_type}'
                    ),
                )
            else:
        ...
        if seg_filter != None and seg_filter != "":
            len_output_files = len(
                sorted(glob.glob(os.path.join(element_output_dir, f"*{output_type_dcmqi}*"), recursive=False))
            )
