...
class HelmChart:
    ...
    def check_container_use(self):
        ...
        if self.kaapana_type == "extenstion-collection":
            ...
            if (
                ...
                container_tag = (
                    f"{BuildUtils.default_registry}/{container_image}:{container_version}"
                )
                self.add_container_by_tag(
                    container_registry=BuildUtils.default_registry,
    ...
    def lint_chart(self, build_version=False):
        ...
        if HelmChart.enable_lint:
            ...
            command = ["helm", "lint"]
            output = run(
                command, stdout=PIPE, stderr=PIPE, universal_newlines=True, timeout=20, cwd=cwd
            )
            if output.returncode != 0:
    ...
    def create_platform_build_files(platform_chart):
        ...
        with alive_bar(
            bar="classic", spinner="crab", dual_line=False, title="Generate Build-Version"
        ) as bar:
    ...
    def create_collection_build_files(collections_chart, platform_build_files_target_dir):
        ...
        collection_container = [
            x for x in BuildUtils.container_images_available if collections_chart.name in x.tag
        ]
    ...
    def build_platform(platform_chart):
        ...
        for i in range(0, container_count):
            ...
            container_to_build = [
                x for x in BuildUtils.container_images_available if x.tag == container_id
            ]
        ...
        if BuildUtils.vulnerability_scan is True:
            ...
            BuildUtils.logger.info("")
            with alive_bar(
                len(successful_built_containers), dual_line=True, title="Vulnerability Scan"
            ) as bar:
                # Init trivy utils
        ...
        if BuildUtils.create_offline_installation is True:
            ...
            micok8s_base_img_json_path = join(
                BuildUtils.kaapana_dir, "build-scripts", "build_helper", "microk8s_images.json"
            )
            ...
            command = (
                ...
                + [
                    build_tag
                    for build_tag in successful_built_containers
                    if not build_tag.startswith("local-only")
                ]
                + [
                    "-o",
                    str(
                        Path(os.path.dirname(platform_chart.build_chart_dir))
                        / f"{platform_chart.name}-{platform_chart.build_version}-containers.tar"
                    ),
                ]
            )
