...
class TrivyUtils:
    ...
    if which("Trivy") is not None:
        ...
        BuildUtils.generate_issue(
            component=suite_tag, name="Check if Trivy is installed", msg="Trivy is not installed", level="ERROR"
        )
    ...
    def create_sbom(self, image):
        command = [
            ...
            "cyclonedx",
            "--output",
            os.path.join(BuildUtils.build_dir, "sbom.json"),
            image,
        ]
    ...
    def create_vulnerability_report(self, image):
        command = [
            ...
            "json",
            "-o",
            os.path.join(BuildUtils.build_dir, "vulnerability_report.json"),
            "--ignore-unfixed",
            "--severity",
        ...
        if output.returncode != 0:
            ...
            BuildUtils.logger.error(output.stderr)
            BuildUtils.generate_issue(
                component=suite_tag, name="Scan image: " + image, msg="Failed to scan image: " + image, level="ERROR"
            )
            ...
            if len(vulnerability_report["Results"]) > 0:
                for target in vulnerability_report["Results"]:
                    if "Vulnerabilities" in target:
                        ...
                        for vulnerability in target["Vulnerabilities"]:
                            compressed_vulnerability_report[target["Target"]]["VulnerabilityID"] = vulnerability[
                                "VulnerabilityID"
                            ]
                            compressed_vulnerability_report[target["Target"]]["Pkg"] = vulnerability["PkgName"]
                            compressed_vulnerability_report[target["Target"]]["InstalledVersion"] = vulnerability[
                                "InstalledVersion"
                            ]
                            compressed_vulnerability_report[target["Target"]]["FixedVersion"] = vulnerability[
                                "FixedVersion"
                            ]
                            compressed_vulnerability_report[target["Target"]]["Severity"] = vulnerability["Severity"]
                            # Not all vulnerabilities have a description
                            if "Description" in vulnerability:
                                compressed_vulnerability_report[target["Target"]]["Description"] = vulnerability[
                                    "Description"
                                ]
    ...
    def check_dockerfile(self, path_to_dockerfile):
        ...
        if not "Results" in dockerfile_report:
            BuildUtils.logger.warning(
                "No results found in dockerfile report. This is weird. Check the Dockerfile: " + path_to_dockerfile
            )
            return
        ...
        for report in dockerfile_report["Results"]:
            if report["MisconfSummary"]["Failures"] > 0:
                ...
                for misconfiguration in report["Misconfigurations"]:
                    if not misconfiguration["CauseMetadata"]["Code"]["Lines"] == None:
                        self.compressed_dockerfile_report[path_to_dockerfile][report["Target"]]["Lines"] = (
                            str(misconfiguration["CauseMetadata"]["StartLine"])
                            + "-"
                            + str(misconfiguration["CauseMetadata"]["EndLine"])
                        )
