...
from build_helper.charts_helper import HelmChart, init_helm_charts, \
    helm_registry_login
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        "-p", "--password", dest="password", default=None, required=False, help="Password"
    )
    parser.add_argument(
        "-bo",
        "--build-only",
    ...
    parser.add_argument(
        "-kd", "--kaapana-dir", dest="kaapaa_dir", default=None, help="Kaapana repo path."
    )
    parser.add_argument(
    ...
    conf_enable_build_kit = (
        1 if "enable_build_kit" in configuration and configuration["enable_build_kit"] else 0
    )
    ...
    push_to_microk8s = (
        args.push_to_microk8s if args.push_to_microk8s != None else conf_push_to_microk8s
    )
    ...
    platform_filter = (
        args.platform_filter.split(",") if args.platform_filter != None else conf_platform_filter
    )
    ...
    parallel_processes = int(
        args.parallel_processes if args.parallel_processes != 2 else conf_parallel_processes
    )
    include_credentials = (
        args.include_credentials if args.include_credentials != None else conf_include_credentials
    )
    no_login = args.no_login
    vulnerability_scan = (
        args.vulnerability_scan if args.vulnerability_scan != None else conf_vulnerability_scan
    )
    ...
    configuration_check = (
        args.configuration_check if args.configuration_check != None else conf_configuration_check
    )
    ...
    container_engine = (
        "docker" if "container_engine" not in configuration else configuration["container_engine"]
    )
    default_registry = (
        configuration["default_registry"] if "default_registry" in configuration else ""
    )
