...
if __name__ == "__main__":
    ...
    parser.add_argument(
        "-el", "--enable-linting", dest="enable_linting", default=None, help="Enable Helm Chart lint & kubeval."
    )
    parser.add_argument(
        "-sp",
    ...
    parser.add_argument(
        "-ee", "--exit-on-error", dest="exit_on_error", default=None, help="Stop build-process if error occurs."
    )
    parser.add_argument(
        "-pf",
        "--plartform-filter",
    ...
    parser.add_argument(
        "-bd", "--build-dir", dest="build_dir", default=None, help="Specify the main Kaapana repo-dir to build from."
    )
    parser.add_argument(
        "-bip",
    ...
    kaapana_dir = (
        args.kaapaa_dir if args.kaapaa_dir != None else os.path.dirname(os.path.dirname(os.path.abspath(__file__)))
    )
    build_dir = (
        args.build_dir if args.build_dir != None else join(dirname(dirname(os.path.realpath(__file__))), "build")
    )
    ...
    conf_platform_filter = (
        configuration["platform_filter"].split(",") if configuration["platform_filter"].replace(" ", "") != "" else []
    )
    ...
    external_source_dirs = (
        args.external_source_dirs.split(",") if args.external_source_dirs != None else conf_external_source_dirs
    )
    build_ignore_patterns = (
        args.build_ignore_patterns.split(",") if args.build_ignore_patterns != None else conf_build_ignore_patterns
    )
    ...
    skip_push_no_changes = (
        args.skip_push_no_changes if args.skip_push_no_changes != None else conf_skip_push_no_changes
    )
